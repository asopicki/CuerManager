use cuer_database;
use cuer_database::models::{Cuecard, NewPlaylistCuecard, Playlist, PlaylistCuecard, PlaylistData};
use diesel::prelude::*;
...
fn playlist_cuecard(i: i32, c_id: i32, conn: &SqliteConnection) -> QueryResult<PlaylistCuecard> {
    ...
    playlist_cuecards
        .filter(playlist_id.eq(i).and(cuecard_id.eq(c_id)))
...
pub fn get_cuecards(p: &Playlist, conn: &SqliteConnection) -> QueryResult<Vec<Cuecard>> {
    cuer_database::schema::playlist_cuecards::table
