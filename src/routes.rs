...
use cuer_database::models::{Event, EventData, Program, ProgramData, Tip, TipData, TipCuecardData };//TipCuecard};
use cuer_database;
...
#[derive(Serialize, Deserialize)]
pub struct FullTip { //Tip including cue cards
	id: i32,
	uuid: String,
	name: String,
	program_id: i32,
...
pub fn delete_event(uuid: String, conn: DbConn) -> Result<Json<Event>, Status> {
	programming::delete_event(&uuid, &conn).map(Json).or_else(|_| Err(Status::NotFound))
}

#[get("/v2/events/<uuid>")]
pub fn event_by_uuid(uuid: String, conn: DbConn) -> Result<Json<Event>, Status> {
	programming::event_by_uuid(&uuid, &conn).map(Json).or_else(|_| Err(Status::NotFound))
}
...
pub fn get_events(conn: DbConn, min_date: String, max_date: String) -> Result<Json<Vec<Event>>, Status> {
	...
	programming::get_events(&conn, start_date.to_rfc3339(), end_date.to_rfc3339()).map(Json).or_else(|_| Err(Status::BadRequest))
}
		...
		.map(|p|  {
			match p {
				Some(p) => {
					...
					let markdown = markdown_to_html(p.notes.unwrap_or_else(|| "".to_string()).as_str(), &options);
					let program = Program {
						id: p.id,
						uuid: p.uuid,
						notes: Some(markdown),
...
pub fn get_tips(program_id: i32, conn: DbConn) -> Result<Json<Vec<FullTip>>, Status> {
	programming::tips_by_program_id(program_id, &conn).map(|tips| {
			let mut result: Vec<FullTip> = Vec::with_capacity(tips.len());
			for tip in tips.into_iter() {
				let cuecards = programming::get_cuecards(&tip, &conn).unwrap_or_else(|_|Vec::new());
...
pub fn create_tip_cuecard(tip_cuecard: Json<FormTipCuecard>, conn: DbConn) -> Result<Json<()>, Status> {
...
#[delete("/v2/tips/<tip_uuid>/cuecard/<cuecard_uuid>", format="application/json")]
pub fn remove_tip_cuecard(tip_uuid: String, cuecard_uuid: String, conn: DbConn) -> Result<Json<()>, Status> {
